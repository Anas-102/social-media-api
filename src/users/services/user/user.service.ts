...
export class UserService {
  ...
  async fetchUserProfiles(username: string) {
    return this.userdb.find({ where: { name: Like(`%${username}%`)}, select:['name','email', 'profileImage'] });
    
  }

  async fetchProfile(user_id: number) {
    return await this.userdb.findOne({ where: { id: user_id } });
  }
  ...
  userLogin = async (credentials: LoginDTO) => {
    ...
    const refresh = await this.rtdb.findOne({
      relations: { user: true },
      where: { user: info },
    });
    console.log(refresh);
